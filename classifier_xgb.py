...
def save_results(results, noise, bias):
    ...
    save_path = os.path.join('evaluation', 'xgb', save_name)
...
def best_model_and_results(X_train, y_train, X_test, y_test):
    ...
    xgb = XGBClassifier()
    clf = GridSearchCV(xgb, PARAMS_OPTIMIZE, n_jobs=-1, scoring='accuracy', cv=10)
...
if __name__ == '__main__':
    ...
    if args.mode is 'single':
        ...
        # 网格搜索，模型评估
        logging.info('Searching best XGB params...'.format(args.noise, args.bias))
    ...
    # 遍历所有数据集
        ...
        for noise in PARAMS_DATASET['noise']:
            for bias in PARAMS_DATASET['bias']:
                ...
                # 网格搜索，效果评估
                logging.info('Searching best XGB params...'.format(noise, bias))
                results = best_model_and_results(X_train, y_train, X_test, y_test)
                print(results['pred_acc'], results['best_params'], results['best_score'])
