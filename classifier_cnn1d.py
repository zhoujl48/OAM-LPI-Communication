...
from keras.layers import Input, Embedding, Conv2D, Reshape, MaxPooling2D, Flatten, Dense, Dropout
from keras.callbacks import ModelCheckpoint, TensorBoard
from utils import log
from FeatureEngineering import DataLoader
from config import WORK_DIR, SIZE_ONE_SAMPLE, NUM_CLASSES, NUM_EACH_CLASS, LABEL_SELECTED
...
class CNNModel(object):
    ...
    def __init__(self, data_split, save_path_base=None, epoch=100, batch_size=128, dropout_size=0.5, tb_log_dir=None, regular=0.001,
                 input_len=30, embedding_size=32, discrete_num=24000, lstm_size=64, dense_size=128, output_size=16, feature_predict=None):
    ...
    def model(self):
        ...
        feature_reshape = Reshape((self._input_len, self._embedding_size, 1))(feature_embedding_dropout)
        input_con2d = Conv2D(filters=64, kernel_size=(3, self._embedding_size))(feature_reshape)
        input_con2d_reshape = Reshape((self._input_len - 3 + 1, 64, 1))(input_con2d)
        input_pooling = MaxPooling2D(pool_size=(1, 64), padding='valid')(input_con2d_reshape)
        input_pooling_reshape = Reshape((self._input_len - 3 + 1,))(input_pooling)
        dense = Dense(self._dense_size, activation='relu', kernel_regularizer=regularizers.l1(self._regular))(input_pooling_reshape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dropout_size', help='set the training regularization', default=0.1, type=float)
    ...
    # 模型保存路径
    model_dir = os.path.join(WORK_DIR, 'model', 'cnn', data_type)
    if not os.path.exists(model_dir):
        logging.info('Makedir: {}'.format(model_dir))
        os.makedirs(model_dir)
    save_dir_base = os.path.join(model_dir, 'cnn_embedding_{embedding_size}_lstm_{lstm_size}_batch_{batch_size}_regular_{regular}_dropout_{dropout}'\
